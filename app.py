...
def render_status():
    ...
    cols[1].metric("Trees", player["trees"])
    cols[2].metric("Need", required_trees_for_current_stage())
    cols[3].metric("Turn", f"{game['turn']}/{settings['turn_limit']}")
    cols[4].metric(
        "Losing turns",
        f"{game['failed_turns']}/{settings['failed_turn_limit']}",
    )
    ...
    for index, (column, stage) in enumerate(zip(columns, STATIONS)):
        ...
        column.markdown(
            ...
            <div class="stage-card {state_class}">
                ...
                <div class="stage-name">{html.escape(stage)}</div>
...
def screen_setup():
    ...
    with st.form("setup_form"):
        ...
        st.caption(FARMS[farm]["description"])
        ...
        st.info(DIFFICULTIES[difficulty]["description"])
